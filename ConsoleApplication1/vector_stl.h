...
public:
    ...
    explicit Vector(size_t count, const T& value = T())
        : data_(nullptr), size_(0), capacity_(0), realloc_count_(0)
    {
        if (count > 0) {
            reserve(count);
...
public:
    ...
    /**
     * @brief Pasalina elementus [first, last) intervale.
     * @return iteratorius i kita elementa po intervalo.
     */
    iterator erase(const_iterator first, const_iterator last)
    {
        size_t start = first - data_;
        size_t stop  = last  - data_;
        if (start > size_ || stop > size_ || start > stop)
            throw std::out_of_range("Vector::erase range out of range");

        size_t count = stop - start;
        for (size_t i = stop; i < size_; i++)
...
/// @brief Free funkcija swap (ADL palaikymui).
template<typename T>
inline void swap(Vector<T>& a, Vector<T>& b) noexcept { a.swap(b); }
